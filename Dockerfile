FROM composer:2.2.4 AS build-env
...
RUN crontab -l | { cat; echo '*/10    *       *       *       *       eval $(printenv | grep -E "^(JIRA|LUMTURIO)_" | sed "s/^\(.*\)$/export \1/g"); /opt/lumturio-jira/bin/lumturio-jira --verbose'; } | crontab -
